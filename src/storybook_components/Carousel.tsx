...
export default function Carousel({elements} : Carousel) {
    ...
    function isAltImage(el: CardProps | AltImageProps | ImageProps): el is AltImageProps {
    ...
    const list = elements.map((el, i) => {
        if(isCard(el)) { 
          ...
          return(
                <li key={i}>
                  <AltImage url={el.url} altText={el.altText} title={el.title} description={el.description} link={el.link}/>
                </li>
                )
        } else {
            return(
